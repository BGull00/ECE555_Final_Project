...
void ESP8266_InitUART(uint32_t baud, int echo){ volatile int delay;
  ...
  while((SYSCTL->PRUART&0x02)==0){};
  SYSCTL->RCGCGPIO |= 0x02; // Enable PORT B clock gating
  while((SYSCTL->PRGPIO&0x02)==0){};
  GPIOB->AFSEL |= 0x03;
  GPIOB->DIR |= 0x20; // PB5 output to reset
  GPIOB->PCTL =(GPIOB->PCTL&0xFF0FFF00)|0x00000011;
  GPIOB->DEN   |= 0x23; //23
  GPIOB->DATA |= 0x20; // reset high
  UART1->CTL &= ~UART_CTL_UARTEN;                  // Clear UART1 enable bit during config
  UART1->IBRD = 50000000/16/baud;
  UART1->FBRD = ((64*((50000000/16)%baud))+baud/2)/baud;      
   // UART1_IBRD_R = 43;       // IBRD = int(80,000,000 / (16 * 115,200)) = int(43.403)
   // UART1_FBRD_R = 26;       // FBRD = round(0.4028 * 64 ) = 26

  UART1->LCRH = (UART_LCRH_WLEN_8|UART_LCRH_FEN);  // 8 bit word length, 1 stop, no parity, FIFOs enabled
  UART1->IFLS &= ~0x3F;                            // Clear TX and RX interrupt FIFO level fields
  UART1->IFLS += UART_IFLS_RX1_8 ;                 // RX FIFO interrupt threshold >= 1/8th full
  UART1->IM  |= UART_IM_RXIM | UART_IM_RTIM;       // Enable interupt on RX and RX transmission end
  UART1->CTL |= UART_CTL_UARTEN;                   // Set UART1 enable bit
}
// -----------UART1_Handler-----------
// called on one receiver data input followed by timeout
// or     on going from 1 to 2 data input characters
void UART1_Handler(void){
  if(UART1->RIS & UART_RIS_RXRIS){   // rx fifo >= 1/8 full
    UART1->ICR = UART_ICR_RXIC;      // acknowledge interrupt
    ESP8266FIFOtoBuffer();
  }
  if(UART1->RIS & UART_RIS_RTRIS){   // receiver timed out
    UART1->ICR = UART_ICR_RTIC;      // acknowledge receiver time
    ESP8266FIFOtoBuffer();
  }
}
...
void ESP8266FIFOtoBuffer(void){
  ...
  while((UART1->FR & UART_FR_RXFE) == 0){
    letter = UART1->DR;        // retrieve char from FIFO
...
// step 1: AT+RST reset module
  UART_OutString("ESP8266 Initialization:\n\r");
