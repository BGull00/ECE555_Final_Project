...
void UART_Init(void){
  ...
  UART0->IBRD = 27;                    // IBRD = int(50,000,000 / (16 * 115,200)) = int(27.126)
  UART0->FBRD = 9;                    // FBRD = round(0.126 * 64 ) = 9
...
void UART_OutChar(char data){
  ...
  UART0->IM &= ~UART_IM_TXIM;          // disable TX FIFO interrupt
...
void UART0_Handler(void){
  ...
  if(UART0->RIS&UART_RIS_RXRIS){       // hardware RX FIFO >= 2 items
    UART0->ICR = UART_ICR_RXIC;        // acknowledge RX FIFO
    // copy from hardware RX FIFO to software RX FIFO
    copyHardwareToSoftware();
  }
  if(UART0->RIS&UART_RIS_RTRIS){       // receiver timed out
